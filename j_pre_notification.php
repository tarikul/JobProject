 ...
 global $a;
  /* return name and id from $job_users table*/
			...
			if ($_REQUEST['t']=="r" )
			{
				extract($_POST);
				$id=$_REQUEST['id'];
				$sql="update $user_notification set `view`='2' where `id`=$id";
				//echo $sql;
				$res=mysql_query($sql);
				if($res)
					{
	...
	<body>
		...
		<div class="row">
			<div class="col-md-10 col-md-offset-1">
				...
				<div class="col-md-9">
					<div class="content">
					<form name="" id="" action="j_pre_notification.php" method="post">
							<div class="table-responsive">
								<table class="table table-bordered" id="datatable-icons" >
									...
									<tbody>
										<?php
										$qry="select * from $user_notification where `user_id`='$user_id' and `view` NOT LIKE '2' ORDER BY `id` DESC";
										$res=mysql_query($qry);
										$no=1;
										if($res)
										{
											while($date=mysql_fetch_array($res))
											{
												...
												$company_name=company_name($conn,$date['com_id']);
												$job_title=job_title_retrive($conn,$date['com_job_id']);
												$p_id=$date[id];
												$view=$date[view];
												
												echo "<tr>";
												echo "<td style='width: 5%; text-align: center;' name='$date[id]'>$no</td>";
												echo "<td style='width: 55%;'>Company Name: $company_name<br>Job Title: $job_title<br>Date: $date[Date] Time: $date[Time]<br>Location: $date[location]</td>";
												echo "<td style='text-align: center;'>$date[update]</td>";
												if($view==0){
												echo "<td style='width: 25%;'>
												<a  class='btn btn-danger' href='j_pre_notification.php?id=$p_id&t=v'>View</a>
												<a class='btn btn-info' href='j_pre_notification.php?id=$p_id&t=r'>Remove</a>
												</td>";
												}
												else
												{
