 ...
 global $a;
  /* return name and id from $job_users table*/
			...
			if ($_REQUEST['t']=="v")
			{
				extract($_POST);
				$id=$_REQUEST['id'];
				$sql="update $com_nofitication  set `view`='1' where `id`=$id";
				//echo $sql;
				$res=mysql_query($sql);
				if($res)
					{
						?>
							<script>
							window.location = "com_notification2.php";
							</script>
						<?php
					}
				exit;
			...
			if ($_REQUEST['t']=="r" )
			{
				extract($_POST);
				$id=$_REQUEST['id'];
				$sql="update $com_nofitication  set `view`='2' where `id`=$id";
				//echo $sql;
				$res=mysql_query($sql);
				if($res)
					{
					...
					<form name="" id="" action="" method="post">
							<div class="table-responsive">
								<table class="table table-bordered" id="datatable-icons" >
									...
									<tbody>
										<?php
										$qry="select * from $com_nofitication where `com_name`='$com_name' and `view` NOT LIKE '2' ORDER BY `id` DESC";
										...
										if($res)
										{
											while($date=mysql_fetch_array($res))
											{
												...
												echo "<td style='width: 10%;'>$date[send_by]</td>";
												echo "<td style='text-align: center;'>$date[message]</td>";
												if($view==0){
												echo "<td style='width: 25%;'>
												<a  class='btn btn-danger' href='com_notification2.php?id=$p_id&t=v'>View</a>
												<a class='btn btn-info' href='com_notification2.php?id=$p_id&t=r'>Remove</a>
												</td>";
												}
												else
												{
